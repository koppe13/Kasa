...
// si nextindex (fin 2) repart a nextindex 0 donc 1ere image comme debut de la serie
  ...
  return (
    <div className="kasa-logement-caroussel" >
      <img onClick={handlePrev} className={`kasa-arrow-left ${getLogement.pictures.length === 1? 'invisible' : '' }`} src={Arrowl} alt="fleche droite"></img>
      ...
      <img onClick={handleNext} className={`kasa-arrow-right ${getLogement.pictures.length === 1? "invisible" : '' }`} src={Arrowr} alt="fleche droite"></img>
      <p className={`carrousel-compteur ${getLogement.pictures.length === 1? "invisible" : '' }`}>{currentIndex +1}/{getLogement.pictures.length}</p>
    </div>
